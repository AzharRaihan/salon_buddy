...
const fetchAttendance = async () => {
    try {
        const res = await $api(`/attendances/${route.query.id}`)
        ...
        console.error(err)
        toast('Error fetching attendance', {
            type: 'error'
        })
    }
}
...
<template>
    <VRow>
        <VCol cols="12">
            <VCard :title="$t('Edit Attendance')">
                <VCardText>
                    <VForm class="mt-3" @submit.prevent="updateAttendance">
                        <VRow>
