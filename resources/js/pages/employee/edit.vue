...
const existingPassword = ref(null)
const ageError = ref('')
...
const validatePassword = (password) => {
    // Only validate password if will_login is Yes and user has no existing password
    if (form.value.will_login == 'Yes' && !existingPassword.value && !password) {
        passwordError.value = 'Password is required for new login users'
        return false
    }
    
    // If password is provided (optional for existing users), validate length
...
const validatePasswordConfirmation = (passwordConfirmation) => {
    if (form.value.password && passwordConfirmation !== form.value.password) {
        passwordConfirmationError.value = 'Password confirmation does not match'
        return false
    }
...
const validateBranch = (branch) => {
    if (!branch || branch.length == 0 && form.value.will_login == 'Yes') {
        branchError.value = 'At least one branch is required'
        return false
    }
    branchError.value = ''
    return true
...
const validateSalary = (salary) => {
    if (!salary) {
        salaryError.value = 'Salary is required'
        return false
    }
    salaryError.value = ''
    return true
...
const validateService = (service) => {
    if (!service || service.length == 0) {
        serviceError.value = 'At least one service is required'
        return false
    }
    serviceError.value = ''
    return true
...
const fetchUserData = async () => {
    try {
        const response = await $api(`/users/${route.query.id}`)
        const userData = response.data
        existingPassword.value = userData.temp_keyword
        ...
        form.value = {
            ...
            photo: userData.photo || defaultAvater,
            age: userData.age || '',
        ...
        // Set preview image
        previewImage.value = userData.photo_url || defaultAvater
        imageSrc.value = userData.photo_url || defaultAvater
    ...
    form.value.social_media.forEach((social, index) => {
        if (social.is_active && !social.url.trim()) {
            ...
            if (!urlPattern.test(social.url.trim())) {
                errors.value.social_media[index] = `Please enter a valid ${social.name} URL`
                isValid = false
            }
        }
    })
...
const validateForm = () => {
    ...
    const isSalaryValid = validateSalary(form.value.salary)
    ...
    return isNameValid && isEmailValid && isPhoneValid && isPasswordValid && 
           ...
           isAgeValid && isGenderValid && isQualificationValid && isExperienceValid && isDescriptionValid
}
...
const updateUser = async () => {
    ...
    try {
        ...
        const res = await $api(`/users/${route.query.id}`, {
            method: 'POST',
            body: formData,
            headers: {
...
<template>
    <VRow>
        <VCol cols="12">
            <VCard :title="t('Edit Employee')">
                <VCardText>
                    <VForm class="mt-3" @submit.prevent="updateUser">
                        <VRow>
                            <!-- Name -->
                            ...
                            <!-- Phone -->
                            <VCol cols="12" md="6" lg="4">
                                <AppTextField v-model="form.phone" :label="t('Phone Number')" :required="true" type="text"
                            ...
                            <!-- Commission -->
                            <VCol cols="12" md="6" lg="4">
                                <AppTextField v-model="form.commission" :label="t('Commission')" type="number" :placeholder="t('Enter Commission')" />
                            </VCol>
                        ...
                        <VRow>
                            ...
                            <VCol cols="12" md="6" lg="4">
                                <VRadioGroup v-model="form.will_login" :label="t('Will Login?')" inline>
                                    <VRadio :label="t('Yes')" value="Yes" :checked="form.will_login == 'Yes'" />
                                    <VRadio :label="t('No')" value="No" :checked="form.will_login == 'No'" />
                                </VRadioGroup>
                            </VCol>
                        </VRow>
                            ...
                            <template v-if="form.will_login == 'Yes'">
                                ...
                                <VCol cols="12" md="6" lg="4">
                                    <AppTextField v-model="form.password" :label="t('Password')" type="password"
                                        :required="true"
                                        :placeholder="existingPassword ? t('Enter password (leave blank to keep current)') : t('Enter Password')"
