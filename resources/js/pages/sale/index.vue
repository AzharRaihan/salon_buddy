...
const TotalPaid = computed(() => t('Total Paid'))
const TotalDue = computed(() => t('Total Due'))
        ...
        <VCard :title="t('List Sale')">
            ...
            <VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page" :items="sales" item-value="id"
                ...
                <template #item.customer="{ item }">
                    {{ item.customer?.name }}
                    <span class="text-muted" v-if="item.customer?.phone">
                        ({{ item.customer?.phone }})
                    </span>
                </template>
                ...
                <template #item.total_payable="{ item }">
                    <span class="font-weight-medium" :class="formatNumberPrecision(item.total_payable) == formatNumberPrecision(item.total_paid) ? 'text-success' : 'text-primary'">
                        {{ formatAmount(item.total_payable) }}
                    </span>
                </template>

                <template #item.total_paid="{ item }">
                    <span class="font-weight-medium" :class="formatNumberPrecision(item.total_payable) != formatNumberPrecision(item.total_paid) && item.total_paid > 0 ? 'text-warning' : ''">
