...
onMounted(async () => {
    try {
        const response = await $api(`/branches/${route.query.id}`)
        ...
        form.value = {
            ...
            active_status: branchData.active_status || null,
            photo: defaultAvater,
            photo_url: branchData.photo_url || defaultAvater,
            open_day_start: branchData.open_day_start || null,
            open_day_end: branchData.open_day_end || null,
            open_day_start_time: branchData.open_day_start_time || null,
            open_day_end_time: branchData.open_day_end_time || null,
        }
        // Set preview image
        previewImage.value = branchData.photo_url || defaultAvater
        imageSrc.value = branchData.photo_url || defaultAvater
    } catch (error) {
...
const updateBranch = async () => {
    ...
    if (!validateName(form.value.branch_name) ||
        !validateCode(form.value.branch_code) ||
        !validateAddress(form.value.address) ||
        !validatePhone(form.value.phone) ||
        !validateActiveStatus(form.value.active_status) ||
        !validateEmail(form.value.email) ||
        !validateOpenDayStart(form.value.open_day_start) ||
        !validateOpenDayEnd(form.value.open_day_end) ||
        !validateOpenDayStartTime(form.value.open_day_start_time) ||
        !validateOpenDayEndTime(form.value.open_day_end_time) ||
        !validatePhoto(form.value.photo)) {
    ...
    try {
        ...
        const res = await $api(`/branches/${route.query.id}`, {
...
<template>
    <VRow>
        <VCol cols="12">
            <VCard :title="$t('Edit Branch')">
                <VCardText>
                    <VForm class="mt-3" @submit.prevent="updateBranch">
                        <VRow>

                            <!-- Branch Code -->
                            <VCol cols="12" md="6" lg="4">
                                <AppTextField v-model="form.branch_code" :label="$t('Branch Code')" :required="true" type="text" :placeholder="$t('Enter branch code')"
                                    :error-messages="branchCodeError" @input="validateCode($event.target.value)" readonly />
                            </VCol>

                            <!-- Branch Name -->
                            <VCol cols="12" md="6" lg="4">
                                <AppTextField v-model="form.branch_name" :label="$t('Branch Name')" :required="true" type="text" :placeholder="$t('Enter branch name')"
                                    :error-messages="branchNameError" @input="validateName($event.target.value)" />
                            </VCol>

                            <!-- Phone -->
                            <VCol cols="12" md="6" lg="4">
                                <AppTextField v-model="form.phone" :label="$t('Phone')" :required="true" type="text"
                                    :placeholder="$t('Enter phone')" :error-messages="phoneError" @input="validatePhone($event.target.value)" />
                            </VCol>

                            <!-- Email -->
                            <VCol cols="12" md="6" lg="4">
                                <AppTextField v-model="form.email" :label="$t('Email')" type="email"
                            ...
                            <!-- Start Day -->
                            <VCol cols="12" md="6" lg="4">
                                <AppAutocomplete v-model="form.open_day_start" :items="['Saturday', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']" :label="$t('Open Day Start')" :required="true" :placeholder="$t('Select Open Day Start')" :error-messages="openDayStartError" @update:model-value="validateOpenDayStart" clearable />
                            </VCol>
                            <!-- End Day -->
                            <VCol cols="12" md="6" lg="4">
                                <AppAutocomplete v-model="form.open_day_end" :items="['Saturday', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']" :label="$t('Open Day End')" :required="true" :placeholder="$t('Select Open Day End')" :error-messages="openDayEndError" @update:model-value="validateOpenDayEnd" clearable />
                            </VCol>

                            <!-- Start Time -->
                            <VCol cols="12" md="6" lg="4">
                                <AppDateTimePicker v-model="form.open_day_start_time" :label="$t('Open Day Start Time')" :required="true" :placeholder="$t('Select time')" :config="{ enableTime: true, noCalendar: true, dateFormat: 'H:i' }" :error-messages="openDayStartTimeError" />
                            </VCol>
                            <!-- End Time -->
                            <VCol cols="12" md="6" lg="4">
                                <AppDateTimePicker v-model="form.open_day_end_time" :label="$t('Open Day End Time')" :required="true" :placeholder="$t('Select time')" :config="{ enableTime: true, noCalendar: true, dateFormat: 'H:i' }" :error-messages="openDayEndTimeError" />
                            </VCol>
