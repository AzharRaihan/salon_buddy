...
const validateForm = () => {
    ...
    const isDescriptionValid = validateNote(form.value.note)

    return isNameValid && isDeliveryChargeValid && isDescriptionValid
}
...
const createDeliveryArea = async () => {
    loadings.value = true
    if (!validateForm()) {
    ...
    try {
        const res = await $api('/delivery-areas', {
            method: 'POST',
            body: form.value,
...
<template>
    <VRow>
        <VCol cols="12">
            <VCard :title="$t('Add Delivery Area')">
                <VCardText>
                    <VForm class="mt-3" @submit.prevent="createDeliveryArea">
                        <VRow>
                            <!-- Name -->
                            ...
                            <VCol cols="12" class="d-flex flex-wrap gap-4">
                                ...
                                <VBtn type="button" @click="router.push({ name: 'website-delivery-area' })" color="primary" variant="tonal">
