...
const fetchWorkingProcess = async () => {
    try {
        const res = await $api(`/working-processes/${route.query.id}`, {
            method: 'GET',
        })
        ...
        console.error(err)
        toast('Error fetching working process data', {
            type: 'error'
        })
        router.push({ name: 'website-workingprocess' })
    }
}
...
<template>
    <VRow>
        <VCol cols="12">
            <VCard :title="$t('Edit Working Process')">
                <VCardText>
                    <VForm class="mt-3" @submit.prevent="updateWorkingProcess">
                        <VRow>
                            ...
                            <VCol cols="12" class="d-flex flex-wrap gap-4">
                                ...
                                <VBtn color="primary" variant="tonal" @click="router.push({ name: 'website-workingprocess' })">
