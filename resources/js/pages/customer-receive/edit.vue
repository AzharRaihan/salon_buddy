...
// Fetch customer receive data
const fetchCustomerReceive = async (id) => {
    try {
        const res = await $api(`/customer-receives/${id}`)
        const data = res.data.customer_receive
        
        // Convert date from YYYY-MM-DD to Month DD, YYYY format
        const dateObj = new Date(data.date)
        const formattedDate = dateObj.toLocaleString('default', { month: 'long' }) + ' ' + 
                             dateObj.getDate() + ', ' +
                             dateObj.getFullYear()
        ...
        form.value = {
            ...
            date: formattedDate,
            amount: data.amount,
            note: data.note || '', // Set empty string if note is null
            customer_id: data.customer?.id,
            payment_method_id: data.payment_method?.id,
            branch_id: branch_info.id
        }
    } catch (err) {
        console.error('Error fetching customer receive:', err)
        toast('Error fetching customer receive data', {
            type: 'error'
        })
    }
}
...
const updateCustomerReceive = async () => {
    loadings.value = true
    if (!validateForm()) {
    ...
    try {
        ...
        Object.keys(form.value).forEach(key => {
            formData.append(key, form.value[key] || '') // Set empty string if value is null
        })
...
<template>
    <VRow>
        <VCol cols="12">
            <VCard :title="t('Edit Customer Receive')">
                <VCardText>
                    <VForm class="mt-3" @submit.prevent="updateCustomerReceive">
                        <VRow>
                            <!-- Reference No -->
                            ...
                            <VCol cols="12" class="d-flex flex-wrap gap-4">
                                ...
                                <VBtn color="primary" variant="tonal" @click="router.push({ name: 'customer-receive' })">
