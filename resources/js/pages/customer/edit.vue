...
const validateForm = () => {
    ...
    if (taxIsGst.value == 'Yes') {
        ...
        return isNameValid && isPhoneValid && isEmailValid && isAddressValid && 
            isDateOfBirthValid && isDateOfAnniversaryValid
    }
}
...
const fetchCustomer = async () => {
    try {
        const res = await $api(`/customers/${route.query.id}`, {
            method: 'GET',
        })
        ...
        form.value = {
            ...
            date_of_anniversary: data.date_of_anniversary ? (data.date_of_anniversary) : '',
        ...
        console.error(err)
        toast(t('Error loading customer data'), {
            type: 'error'
        })
...
const updateCustomer = async () => {
    ...
    try {
        ...
        Object.keys(form.value).forEach(key => {
            if (key == 'photo') {
                if (form.value[key] !== defaultAvater) {
...
<template>
    <VRow>
        <VCol cols="12">
            <VCard :title="$t('Edit Customer')">
                <VCardText>
                    <VForm class="mt-3" @submit.prevent="updateCustomer">
                        <VRow>
                            <!-- Name -->
                            ...
                            <VCol cols="12" class="d-flex flex-wrap gap-4">
                                ...
                                <VBtn color="primary" variant="tonal" @click="router.push({ name: 'customer' })">
