...
const expenses = computed(() => {
    const data = expenseData.value?.expenses || []
    return data.map((expense, index) => ({
        ...expense,
        serial_number: getSerialNumber(index, totalExpenses.value, page.value, itemsPerPage.value),
    }))
})
const totalExpenses = computed(() => expenseData.value?.total || 0)

const openConfirmDialog = (expenseId) => {
...
const handleDelete = async (confirmed) => {
    ...
    try {
        await $api(`/expenses/${selectedExpenseId.value}`, {
            method: 'DELETE',
        })
        selectedExpenseId.value = null
        isConfirmDialogOpen.value = false
        await fetchExpenses()
        toast(t('Expense deleted successfully'), {
            "type": "success",
        });
    } catch (error) {
        isConfirmDialogOpen.value = false
        ...
        toast(t('Failed to delete expense'), {
            "type": "error",
        });
    }
}
...
<template>
    <div>
        <VCard :title="t('List Expense')">
            <VCardText>
                <div class="d-flex justify-space-between flex-wrap gap-y-4">
                    <AppTextField v-model="searchQuery" style="max-inline-size: 280px; min-inline-size: 280px;"
                        :placeholder="t('Search Expense')" />
                    <div class="d-flex flex-row gap-4 align-center flex-wrap">
                        <VBtn prepend-icon="tabler-plus" :to="{ name: 'expense-create' }">
                            {{ t('Add Expense') }}
                        </VBtn>
            ...
            <VDivider />
            <VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page" :items="expenses" item-value="id"
                :headers="headers" :items-length="totalExpenses" class="text-no-wrap" @update:options="updateOptions">
                ...
                <template #item.action="{ item }">
                    <div class="d-flex justify-center gap-1">
                        <VBtn icon variant="text" color="info" size="small"
                            @click="$router.push({ name: 'expense-edit', query: { id: item.id } })">
        ...
        <ConfirmDialog v-model:is-dialog-visible="isConfirmDialogOpen"
            :confirmation-question="$t('Are you sure you want to delete this expense?')" :confirm-title="$t('Deleted!')"
            :confirm-msg="$t('Expense has been deleted successfully.')" :cancel-title="$t('Cancelled')"
            :cancel-msg="$t('Expense Deletion Cancelled!')" @confirm="handleDelete" />
    </div>
</template>
