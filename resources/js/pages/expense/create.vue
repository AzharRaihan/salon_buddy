...
const fetchExpenseReferenceNo = async () => {
    try {
        const res = await $api('/generate-expense-reference-no')
        form.value.reference_no = res.data
    } catch (err) {
        console.error('Error fetching expense reference no:', err)
        toast('Error generating expense reference no', {
            type: 'error'
        })
    }
}
