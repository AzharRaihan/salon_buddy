...
const units = computed(() => {
    const data = unitData.value?.units || []
    return data.map((unit, index) => ({
        ...unit,
        serial_number: getSerialNumber(index, totalUnits.value, page.value, itemsPerPage.value),
    }))
})
const totalUnits = computed(() => unitData.value?.total || 0)

const openConfirmDialog = (unitId) => {
...
const handleDelete = async (confirmed) => {
    ...
    try {
        await $api(`/units/${selectedUnitId.value}`, {
            method: 'DELETE',
        })
...
<template>
    <div>
        <VCard :title="$t('List Unit')">
            <VCardText>
                <div class="d-flex justify-space-between flex-wrap gap-y-4">
                    <AppTextField v-model="searchQuery" style="max-inline-size: 280px; min-inline-size: 280px;"
                        :placeholder="$t('Search Unit')" />
                    <div class="d-flex flex-row gap-4 align-center flex-wrap">
                        <VBtn prepend-icon="tabler-plus" :to="{ name: 'unit-create' }">
                            {{ $t('Add Unit') }}
                        </VBtn>
            ...
            <VDivider />
            <VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page" :items="units" item-value="id"
                :headers="headers" :items-length="totalUnits" class="text-no-wrap" @update:options="updateOptions">
                ...
                <template #item.action="{ item }">
                    <div class="d-flex justify-center gap-1">
                        <VBtn icon variant="text" color="info" size="small"
                            @click="$router.push({ name: 'unit-edit', query: { id: item.id } })">
        ...
        <ConfirmDialog v-model:is-dialog-visible="isConfirmDialogOpen"
            :confirmation-question="$t('Are you sure you want to delete this unit?')" :confirm-title="$t('Deleted!')"
            :confirm-msg="$t('Unit has been deleted successfully.')" :cancel-title="$t('Cancelled')"
            :cancel-msg="$t('Unit Deletion Cancelled!')" @confirm="handleDelete" />
    </div>
</template>
