...
const fetchUnit = async () => {
    try {
        const res = await $api(`/units/${route.query.id}`, {
            method: 'GET',
        })
        ...
        console.error(err)
        toast('Error fetching unit data', {
            type: 'error'
        })
...
<template>
    <VRow>
        <VCol cols="12">
            <VCard :title="$t('Edit Unit')">
                <VCardText>
                    <VForm class="mt-3" @submit.prevent="updateUnit">
                        <VRow>
                            <!-- Name -->
                            <VCol cols="12" md="6" lg="4">
                                <AppTextField v-model="form.name" :label="$t('Unit Name')" :required="true" type="text" :placeholder="$t('Enter Unit Name')"
                            ...
                            <VCol cols="12" class="d-flex flex-wrap gap-4">
                                ...
                                <VBtn color="primary" variant="tonal" @click="router.push({ name: 'unit' })">
