  ...
  if (props.summaryData) {
    ...
    props.headers.forEach(header => {
      if (header.key === 'sn') {
        ...
        summaryRow[header.title] = props.summaryData?.totalDebit || 0
      } else if (header.key === 'credit') {
        summaryRow[header.title] = props.summaryData?.totalCredit || 0
      } else {
        summaryRow[header.title] = ''
      }
    })
    ...
    props.headers.forEach(header => {
      if (header.key === 'title') {
        differenceRow[header.title] = 'DIFFERENCE'
      } else if (header.key === 'debit') {
        differenceRow[header.title] = Math.abs(props.summaryData?.difference || 0)
      } else {
        differenceRow[header.title] = ''
      }
    })
    exportData.push(differenceRow)
  }
