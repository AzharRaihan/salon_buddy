...
const formatDataForExport = () => {
  ...
  if (props.summaryData) {
    ...
    props.headers.forEach(header => {
      if (header.key != 'actions') {
        if (header.key == 'out_time') {
          summaryRow[header.title] = 'Total:'
        } else if (header.key == 'total_time') {
          
          summaryRow[header.title] = props.summaryData?.totalHours || 0 + ' Minutes,  ' + (props.summaryData?.totalHours || 0) / 60 + ' Hours'
        } else {
...
const exportToCSV = () => {
  try {
    ...
    if (props.headerData.employeeName) {
      headerInfo.push(`Employee: ${props.headerData.employeeName}`)
    }
    if (props.headerData.employeePhone) {
      headerInfo.push(`Phone: ${props.headerData.employeePhone}`)
    }

    if(!props.headerData.employeeName) {
      headerInfo.push('All Employees')
    }
...
const exportToPDF = () => {
  try {
    ...
    if (props.headerData.employeeName) {
      doc.text(`Employee: ${props.headerData.employeeName}`, 14, currentY)
      currentY += 6
    }
    if (props.headerData.employeePhone) {
      doc.text(`Phone: ${props.headerData.employeePhone}`, 14, currentY)
      currentY += 6
    }

    if(!props.headerData.employeeName) {
      doc.text('All Employees', 14, currentY)
      currentY += 6
    }
    
    // Generated Info (Right Side)
    doc.text(`Date Range: ${props.headerData.dateRange}`, pageWidth - 14, leftStartY, { align: 'right' })
