...
const exportToCSV = () => {
  try {
    ...
    const headerInfo = [
      ...
      '',
      `Employee: ${props.headerData.employeeName}`
    ]
    
    // Add employee phone only if it exists (specific employee selected)
...
const exportToExcel = async () => {
  try {
    ...
    // Add table headers
    XLSX.utils.sheet_add_aoa(ws, [headers], { origin: `A${headerInfo.length + 1}` })
