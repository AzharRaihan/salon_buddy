...
// Local search query for the input
...
let searchInputTimeout = ref(null)

// Perform search
const performSearch = () => {
  if (localSearchQuery.value.trim()) {
    searchQuery.value = localSearchQuery.value.trim()
    performSearchComposable(localSearchQuery.value.trim(), searchType.value)
  }
}
...
onMounted(() => {
  ...
  if (queryToUse) {
    localSearchQuery.value = queryToUse
